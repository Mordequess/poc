#! /bin/sh
#############################################################################
# Cisco1841_MRTG_CfgGen.sh written by Michael Cole 2013/09/02 for Logiq3
# ---------------------------------------------------------------------------
# Purpose:
...
IfType2Skip="Null Vlan Embedded-Service-Engine NVI0"
...
TokenCount=`echo $MRTGOutfile | awk -F. '{print NF}'`
counter=1
HTML=""
while (( counter < TokenCount ))
do
  HTML="$HTML `echo $MRTGOutfile | awk -F. '{print $cnt}' cnt=$counter`"
  HTML=`echo $HTML | tr ' ' '.'`
  (( counter += 1 ))
done
...
do
  ...
  # On the router seems to be less random than on Catalysts, but to be safe we'll assume that's not always the case
