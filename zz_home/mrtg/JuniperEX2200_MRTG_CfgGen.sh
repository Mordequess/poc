#! /bin/sh
#############################################################################
# JuniperEX2200_MRTG_CfgGen.sh written by Michael Cole 2013/08/27 for Logiq3
# ---------------------------------------------------------------------------
# Purpose:
...
IfType2Skip="tunnel propVirtual mplsTunnel other softwareLoopback me0 bme0 vlan"
						# Space demlimited list of all types of interfaces we won't put in our MRTG config file
...
do
  ...
  # Need the SNMP interface ID number, typically >500, but seems somewhat random.
  ...
  do
    ...
    SkipNotNull=`echo $IfType | grep $If2Skip`
    SkipNotNull2=`echo $Descript | grep $If2Skip`
    ...
    elif [ -n "$SkipNotNull2" ] ; then
      # Interface descriptor name is in the skip list (annoying thing about Junipers some management
      # ports are given a free pass) in the ifType field they are the same type as regular ethernet ports
      SubIndex=$NumTypesToSkip
      SkipNotNull="$SkipNotNull2"
    fi
    (( SubIndex += 1))
  done
  if [ -n "$SkipNotNull" ] ; then
  ...
  # Now we need to add this interface to our config file.
  echo -e "\n" >> $MRTGOutfile
  ...
  if (( OperSpeed == 0 )) ; then
    # Reason number a bajillion a six why I hate Juniper, if the interface is down instead of ifSpeed.x being
    # say, 10Mbps, it's ZERO, which buggers up MRTG becuase then MAXbytes is Zero. So lets make it 10M.
    OperSpeed=10000000
  fi
