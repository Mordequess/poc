#! /bin/sh
#############################################################################
# SMTPLog.sh written by Michael Cole 2013/10/29 for Logiq3
# ---------------------------------------------------------------------------
# Purpose:
# This script will look for a specific IP in the SMTP logs and if that IP
...
# Will update (if needed) the SMTP history for that IP.
# ---------------------------------------------------------------------------
# Arguements:
...
SMTPLog=$ScriptsDir/PingData		# location of all SMTP events that have happened in the last hour
SMTPHist=$ScriptsDir/SMTPHistory	# location of all SMTP logs, i.e. state changes
...
HistFile=$SMTPHist/"$System2Check"_hist.txt	# system history
...
cat $SMTPLog/Ping.* 2> /dev/null | grep $System2Check,.*SMTP > $TmpFile
if [ ! -e $HistFile ] ; then
...
$ScriptsDir/GenUpTimeHTML.sh $HistFile SMTP
rm $TmpFile
echo $RetVal
exit $RetVal
