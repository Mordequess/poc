#! /bin/sh
#############################################################################
# HTTPSLog.sh written by Michael Cole 2013/11/05 for Logiq3
# ---------------------------------------------------------------------------
# Purpose:
# This script will look for a specific IP in the HTTPS logs and if that IP
...
PingHist=$ScriptsDir/HTTPSHistory	# location of all HTTPS logs, i.e. state changes
...
cat $PingLog/Ping.* 2> /dev/null | grep $System2Check,.*HTTPS > $TmpFile
if [ ! -e $HistFile ] ; then
...
$ScriptsDir/GenUpTimeHTML.sh $HistFile HTTPS
rm $TmpFile
echo $RetVal
exit $RetVal
