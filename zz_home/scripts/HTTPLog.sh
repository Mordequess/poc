#! /bin/sh
#############################################################################
# HTTPLog.sh written by Michael Cole 2013/11/05 for Logiq3
# ---------------------------------------------------------------------------
# Purpose:
# This script will look for a specific IP in the HTTP logs and if that IP
...
PingHist=$ScriptsDir/HTTPHistory	# location of all HTTP logs, i.e. state changes
...
cat $PingLog/Ping.* 2> /dev/null | grep $System2Check,.*HTTP > $TmpFile
if [ ! -e $HistFile ] ; then
...
$ScriptsDir/GenUpTimeHTML.sh $HistFile HTTP
rm $TmpFile
echo $RetVal
exit $RetVal
